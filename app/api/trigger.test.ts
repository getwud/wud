...
describe('API Trigger', () => {
    ...
    test('should get all triggers', async () => {
        ...
        expect(res.body).toEqual([
            { id: 'mock.fail', type: 'mock', name: 'fail', configuration: { mockConfig: true } },
    ...
    test('should run trigger successfully', async () => {
        const res = await request(app).post('/mock/test').send({ containerName: 'test-container' });
        expect(res.status).toBe(200);
    });

    test('should return 404 if trigger not found when running', async () => {
        const res = await request(app).post('/mock/unknown').send({ containerName: 'test-container' });
        expect(res.status).toBe(404);
        expect(res.body).toEqual({ error: 'Error when running trigger mock.unknown (trigger not found)' });
    });
    ...
    test('should return 400 if no container provided', async () => {
        ...
        expect(res.body).toEqual({ error: 'Error when running trigger mock.test (container is undefined)' });
    });
    
    test('should handle trigger run failure', async () => {
        const res = await request(app).post('/mock/fail').send({ containerName: 'test-container' });
        expect(res.status).toBe(500);
        expect(res.body).toEqual({ error: 'Error when running trigger mock.fail (fail error)' });
    });
});
